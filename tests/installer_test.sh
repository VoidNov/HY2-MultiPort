...
make_release_fixture() {
    local version=0.0.4 target=x86_64-unknown-linux-gnu
...
if run_installer "$invalid_option_output" status --version 0.0.4; then
...
run_installer "$dry_run_output" --dry-run --version v0.0.4 --base-url "file://$fixture_root/releases"
assert_contains '版本 v0.0.4' "$dry_run_output"
...
run_installer "$install_output" --version 0.0.4 --base-url "file://$fixture_root/releases"
...
run_stdin_installer "$fresh_output" --version 0.0.4 --base-url "file://$fixture_root/releases"
assert_contains '首次使用：5 分钟配置向导' "$fresh_output"
assert_contains 'sudo ' "$fresh_output"
assert_contains 'port-forward init' "$fresh_output"
assert_not_exists "$fixture_root/etc/port-forward/config.toml"
